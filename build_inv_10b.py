...
(function(){
  ...
  function onMove(e){
    ...
    var minSize=24;
    if(d.mode==='move'){
      CROP_STATE.x=Math.max(0,Math.min(CROP_STATE.dispW-CROP_STATE.w,d.origX+dx));
      CROP_STATE.y=Math.max(0,Math.min(CROP_STATE.dispH-CROP_STATE.h,d.origY+dy));
    }else{
      var nx=d.origX,ny=d.origY,nw=d.origW,nh=d.origH;
      if(d.mode==='resize-se'){nw=Math.max(minSize,d.origW+dx);nh=Math.max(minSize,d.origH+dy);}
      else if(d.mode==='resize-sw'){nw=Math.max(minSize,d.origW-dx);nx=d.origX+d.origW-nw;nh=Math.max(minSize,d.origH+dy);}
      else if(d.mode==='resize-ne'){nw=Math.max(minSize,d.origW+dx);nh=Math.max(minSize,d.origH-dy);ny=d.origY+d.origH-nh;}
      else if(d.mode==='resize-nw'){nw=Math.max(minSize,d.origW-dx);nx=d.origX+d.origW-nw;nh=Math.max(minSize,d.origH-dy);ny=d.origY+d.origH-nh;}
      CROP_STATE.x=Math.max(0,nx); CROP_STATE.y=Math.max(0,ny);
      CROP_STATE.w=Math.min(nw,CROP_STATE.dispW-CROP_STATE.x);
      CROP_STATE.h=Math.min(nh,CROP_STATE.dispH-CROP_STATE.y);
    }
    cropRenderRect();
    e.preventDefault();
  }
