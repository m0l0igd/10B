...
.nt{font-weight:700;font-size:1rem;color:#fff;white-space:nowrap}
...
.hp.hid{display:none}
.sep{width:1px;background:rgba(255,255,255,.2);align-self:stretch}
...
<nav>
  ...
  <div class="nt">&#128230; Region 10B Parts Inventory</div>
...
<div class="hbar">
  <div class="hc"><div class="hl">Regional Manager</div><div class="hps" id="rmp"></div></div>
  <div class="sep"></div>
  <div class="hc"><div class="hl">FS Manager</div><div class="hps" id="mgp"></div></div>
</div>
<div class="fb">
...
function bP(){
  var vm=F.rm?(RM_MGRS[F.rm]||[]):ALL_MGRS;
  var rh='<button class="hp '+(F.rm?'':'on')+'" onclick="sRM(&#39;&#39;)">All</button>';
  ALL_RMS.forEach(function(r){rh+='<button class="hp '+(F.rm===r?'on':'')+'" onclick="sRM(&#39;'+esc(r)+'&#39;)">'+esc(r)+'</button>';});
  ge('rmp').innerHTML=rh;
  var mh='<button class="hp '+(F.mgr?'':'on')+'" onclick="sMGR(&#39;&#39;)">All</button>';
  ALL_MGRS.forEach(function(m){mh+='<button class="hp'+(F.mgr===m?' on':'')+(vm.indexOf(m)<0?' hid':'')+'" onclick="sMGR(&#39;'+esc(m)+'&#39;)">'+esc(m)+'</button>';});
  ge('mgp').innerHTML=mh;
}
