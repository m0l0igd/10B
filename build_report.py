...
gm AS (
  SELECT
    aligned_gm_tech AS tech_name, 'GM' AS role, aligned_fs_mgr,
...
hvacr AS (
  SELECT
    aligned_hvacr_tech AS tech_name, 'HVACR' AS role, aligned_fs_mgr,
    DATETIME_DIFF(completion_date, call_date, MINUTE) / 1440.0 AS dtc_calc,
    CASE WHEN high_priority = 'Yes'
      THEN DATETIME_DIFF(completion_date, call_date, MINUTE) / 1440.0
    END AS dtc_hp_calc,
...
fe AS (
  SELECT
    aligned_food_equipment_tech AS tech_name, 'FE' AS role, aligned_fs_mgr,
    DATETIME_DIFF(completion_date, call_date, MINUTE) / 1440.0 AS dtc_calc,
    CASE WHEN high_priority = 'Yes'
      THEN DATETIME_DIFF(completion_date, call_date, MINUTE) / 1440.0
    END AS dtc_hp_calc,
...
FROM combined
GROUP BY tech_name, role, aligned_fs_mgr
ORDER BY role, dtc
"""
...
def row_to_tech(row: Any) -> dict[str, Any]:
    ...
    # Clean the manager name dynamically to resolve alignment errors!
    raw_mgr = row.aligned_fs_mgr if hasattr(row, 'aligned_fs_mgr') and row.aligned_fs_mgr else "MICHAEL LEANOX"
    mgr_upper = raw_mgr.upper()
    if "366-A" in mgr_upper or "ANTONIO" in mgr_upper or "TONY" in mgr_upper:
        manager = "ANTONIO VASQUEZ"
    else:
        manager = "MICHAEL LEANOX"
    ...
    return {
        "name": row.tech_name,
        "role": row.role,
        "manager": manager,  # Cleaned, accurate manager alignment!
...
def main() -> None:
    ...
    for row in rows:
        name_upper = row.tech_name.upper() if row.tech_name else ""
        # Exclude generic database placeholder technicians
        if name_upper in ["1", "366-A-FE", "366-A-FS", "367-A-FS", ""]:
            continue
